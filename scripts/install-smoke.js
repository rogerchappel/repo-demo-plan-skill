...
const tmpDir = mkdtempSync(path.join(tmpdir(), "repo-demo-plan-install-"));
...
try {
  ...
  const binPath = path.join(tmpDir, "node_modules", ".bin", "repo-demo-plan");
  const help = run(binPath, ["--help"], { cwd: tmpDir });

  if (!help.stdout.includes("Usage: repo-demo-plan")) {
    console.error("install smoke failed; installed CLI did not print usage");
    process.exit(1);
  }

  console.log("install smoke passed; installed tarball and ran repo-demo-plan --help");
} finally {
